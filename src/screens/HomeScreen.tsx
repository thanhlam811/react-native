...
const HomeScreen = () => {
  ...
  const renderBookItem = ({ item }: { item: Book }) => (
    <View style={styles.bookCard}>
      <Image source={{ uri: `http://10.0.2.2:8080/uploads/${item.image}` }} style={styles.bookImage} />
      <Text style={styles.bookTitle}>{item.title}</Text>
      <Text style={styles.bookPrice}>{item.sellingPrice}đ</Text>
      <View style={styles.ratingRow}>
        <Icon name="star" size={16} color="#FFD700" />
        <Text>4.5</Text>
        <Text style={{ marginLeft: 'auto' }}>Sold: 100</Text>
      </View>
    </View>
  );
  ...
  return (
    ...
    <ScrollView style={styles.container}>
      ...
      {/* You can reuse FlatList here with filtered purchased books */}

      <View style={styles.sectionHeader}>
        <Text style={styles.sectionTitle}>On your Wishlist</Text>
        <Text style={styles.seeAll}>See all</Text>
      </View>
      {/* You can reuse FlatList here with filtered wishlist books */}
    </ScrollView>
    </SafeAreaView>
  );
};
